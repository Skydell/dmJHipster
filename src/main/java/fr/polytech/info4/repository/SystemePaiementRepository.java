...
public interface SystemePaiementRepository extends JpaRepository<SystemePaiement, Long> {
    ...
    @Query(value = "select distinct systemePaiement from SystemePaiement systemePaiement left join fetch systemePaiement.comptes",
    ...
    @Query("select distinct systemePaiement from SystemePaiement systemePaiement left join fetch systemePaiement.comptes")
    List<SystemePaiement> findAllWithEagerRelationships();

    @Query("select systemePaiement from SystemePaiement systemePaiement left join fetch systemePaiement.comptes where systemePaiement.id =:id")
