...
export class RestaurantUpdatePage {
  ...
  productsInput = element(by.id('field_products'));
  ...
  async setProductsInput(products: string): Promise<void> {
    await this.productsInput.sendKeys(products);
  }

  async getProductsInput(): Promise<string> {
    return await this.productsInput.getAttribute('value');
  }
