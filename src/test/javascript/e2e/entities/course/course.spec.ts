...
describe('Course e2e test', () => {
  ...
  it('should load Courses', async () => {
    ...
    expect(await courseComponentsPage.getTitle()).to.eq('dmJHipsterApp.course.home.title');
  ...
  it('should load create Course page', async () => {
    ...
    expect(await courseUpdatePage.getPageTitle()).to.eq('dmJHipsterApp.course.home.createOrEditLabel');
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([
      courseUpdatePage.setClientnameInput('clientname'),
      courseUpdatePage.setCoursiernameInput('coursiername'),
      courseUpdatePage.setTimerequiredInput('5'),
      courseUpdatePage.panierSelectLastOption(),
      courseUpdatePage.compteSelectLastOption(),
      courseUpdatePage.restaurantSelectLastOption(),
    ]);

    expect(await courseUpdatePage.getClientnameInput()).to.eq('clientname', 'Expected Clientname value to be equals to clientname');
    expect(await courseUpdatePage.getCoursiernameInput()).to.eq('coursiername', 'Expected Coursiername value to be equals to coursiername');
  ...
  it('should delete last Course', async () => {
    ...
    expect(await courseDeleteDialog.getDialogTitle()).to.eq('dmJHipsterApp.course.delete.question');
